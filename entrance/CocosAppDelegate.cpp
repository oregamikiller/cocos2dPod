...
{
    //  this->scriptSource = "main.js";
    //  CCLOG("AppDelegate::AppDelegate scriptSource %s", this->scriptSource.c_str());
}
...
bool CocosAppDelegate::applicationDidFinishLaunching()
{
    ...
    if(!glview) {
        ...
        glview = cocos2d::GLViewImpl::create("testCocos");
#else
        glview = cocos2d::GLViewImpl::createWithRect("testCocos", Rect(0,0,960,640));
#endif
    ...
    if (!this->scriptSource.empty()) {
        scriptSource = this->scriptSource;
    } else {
        scriptSource = "main.js";
    }
    
    CCLOGERROR("Application::applicationDidFinishLaunching scriptSource %s", scriptSource.c_str());
    
    ScriptingCore::getInstance()->runScript(scriptSource);
    
    return true;
}

void CocosAppDelegate::setScriptSource(std::string scriptSource)
{
    this->scriptSource = scriptSource;
    CCLOGERROR("AppDelegate::setScriptSource %s", this->scriptSource.c_str());
